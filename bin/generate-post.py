...
class BlogPost:
    ...
    def _adjust_line_numbers(self, start_line: int, end_line: int) -> (int, int):
        if self._front_matter:
            num_front_matter_lines = len(self._front_matter)+1
            return max(0, start_line - num_front_matter_lines), min(len(self._content) - 1, end_line - num_front_matter_lines)
        else:
            return start_line, end_line

    def join_content(self, start_line: int = 0, end_line: int = -1) -> str:
        start_line, end_line = self._adjust_line_numbers(start_line, end_line)
        return ''.join(self._content[start_line:end_line+1])
    ...
    def update_content(
        ...
        start_line, end_line = self._adjust_line_numbers(start_line, end_line)
        self._content = self._content[:start_line] + new_content + self._content[end_line+1:]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start-line', type=int, default=0, help='The starting line number (optional)')
    parser.add_argument('--end-line', type=int, default=-1, help='The ending line number (optional)')

    args = parser.parse_args()

    blog_post_generator = BlogPostGenerator(args.postname, args.instructions)
    blog_post_generator.generate_post(args.start_line, args.end_line)